...
const gigInclude = { business: { select: { id: true, name: true, businessProfile: { select: { businessName: true } } } }, applications: { select: { id: true, studentId: true, status: true } }, deliverables: { orderBy: { submittedAt: "desc" as const } }, revisionRequests: { orderBy: { requestedAt: "desc" as const } }, payment: true } satisfies Prisma.GigInclude;
...
export async function listApplicants(req: Request, res: Response) {
  ...
  const applicants = await prisma.application.findMany({ where: { gigId: gig.id }, include: { student: { select: { id: true, name: true, email: true, studentProfile: { select: { college: true, skills: true, bio: true, profileImageUrl: true, avgRating: true, totalRatings: true } } } } }, orderBy: { createdAt: "asc" } });
  // Completed-gig counts for every applicant in one grouped query (no N+1).
  const completedCounts = await prisma.application.groupBy({ by: ["studentId"], where: { studentId: { in: applicants.map((a) => a.studentId) }, status: ApplicationStatus.SELECTED, gig: { status: { in: [GigStatus.APPROVED, GigStatus.PAID, GigStatus.CLOSED] } } }, _count: { _all: true } });
  const pastGigCountByStudent = new Map(completedCounts.map((row) => [row.studentId, row._count._all]));
  return res.json({ applicants: applicants.map((application) => ({ ...application, avgRating: application.student.studentProfile?.avgRating ?? 0, totalRatings: application.student.studentProfile?.totalRatings ?? 0, pastGigCount: pastGigCountByStudent.get(application.studentId) ?? 0 })) });
}
...
export async function selectApplicant(req: Request, res: Response) {
  ...
  const newlyRejected = await prisma.application.findMany({ where: { gigId: gig.id, id: { not: application.id }, status: { not: ApplicationStatus.REJECTED } }, select: { studentId: true } });
  await prisma.$transaction([prisma.application.updateMany({ where: { gigId: gig.id, id: { not: application.id } }, data: { status: ApplicationStatus.REJECTED } }), prisma.application.update({ where: { id: application.id }, data: { status: ApplicationStatus.SELECTED } }), prisma.gig.update({ where: { id: gig.id }, data: { status: GigStatus.ASSIGNED } })]);
  notifySafely({ userId: application.studentId, type: NotificationType.APPLICATION_SELECTED, message: `You were selected for "${gig.title}"`, relatedGigId: gig.id });
  for (const rejected of newlyRejected) notifySafely({ userId: rejected.studentId, type: NotificationType.APPLICATION_REJECTED, message: `Your application for "${gig.title}" was not selected`, relatedGigId: gig.id });
  return res.json({ application: await prisma.application.findUnique({ where: { id: application.id } }) });
}

export async function rejectApplicant(req: Request, res: Response) {
  if (!await requireRole(req.userId!, Role.BUSINESS)) return res.status(403).json({ message: "Business access required" }); const application = await prisma.application.findUnique({ where: { id: idParam(req.params.id) }, include: { gig: true } }); if (!application) return res.status(404).json({ message: "Application not found" }); if (application.gig.businessId !== req.userId) return res.status(403).json({ message: "You do not own this gig" }); if (application.gig.status !== GigStatus.OPEN) return res.status(409).json({ message: "Applicants can only be rejected while a gig is open" }); const rejected = await prisma.application.update({ where: { id: application.id }, data: { status: ApplicationStatus.REJECTED } }); notifySafely({ userId: application.studentId, type: NotificationType.APPLICATION_REJECTED, message: `Your application for "${application.gig.title}" was not selected`, relatedGigId: application.gigId }); return res.json({ application: rejected });
}

export async function startGig(req: Request, res: Response) { if (!await requireRole(req.userId!, Role.STUDENT)) return res.status(403).json({ message: "Student access required" }); const gig = await assignedGig(idParam(req.params.id), req.userId!); if (!gig) return res.status(403).json({ message: "You are not assigned to this gig" }); if (gig.status !== GigStatus.ASSIGNED) return res.status(409).json({ message: "Only assigned gigs can be started" }); const payment = await prisma.payment.findUnique({ where: { gigId: gig.id } }); if (payment?.status !== PaymentStatus.HELD) return res.status(409).json({ message: "The business must fund this gig before work can start" }); const started = await prisma.gig.update({ where: { id: gig.id }, data: { status: GigStatus.IN_PROGRESS }, include: gigInclude }); notifySafely({ userId: gig.businessId, type: NotificationType.GIG_STATUS_CHANGED, message: `Work started on "${gig.title}"`, relatedGigId: gig.id }); return res.json({ gig: started }); }

export async function submitDeliverable(req: Request, res: Response) { try { if (!await requireRole(req.userId!, Role.STUDENT)) return res.status(403).json({ message: "Student access required" }); const gig = await assignedGig(idParam(req.params.id), req.userId!); if (!gig) return res.status(403).json({ message: "You are not assigned to this gig" }); if (gig.status !== GigStatus.IN_PROGRESS && gig.status !== GigStatus.REVISION_REQUESTED) return res.status(409).json({ message: "This gig is not ready for a deliverable" }); const body = req.body as Record<string, unknown>; const fileUrl = text(body.fileUrl, "fileUrl"); const note = text(body.note, "note", false); const [, updated] = await prisma.$transaction([prisma.deliverable.create({ data: { gigId: gig.id, fileUrl, note } }), prisma.gig.update({ where: { id: gig.id }, data: { status: GigStatus.SUBMITTED }, include: gigInclude })]); notifySafely({ userId: gig.businessId, type: NotificationType.GIG_STATUS_CHANGED, message: `New deliverable submitted for "${gig.title}"`, relatedGigId: gig.id }); return res.status(201).json({ gig: updated }); } catch (error) { return res.status(400).json({ message: error instanceof Error ? error.message : "Could not submit deliverable" }); } }

export async function requestRevision(req: Request, res: Response) { try { if (!await requireRole(req.userId!, Role.BUSINESS)) return res.status(403).json({ message: "Business access required" }); const gig = await ownedGig(idParam(req.params.id), req.userId!); if (!gig) return res.status(404).json({ message: "Gig not found" }); if (gig.status !== GigStatus.SUBMITTED) return res.status(409).json({ message: "Only submitted gigs can be sent for revision" }); const feedback = text((req.body as Record<string, unknown>).feedback, "feedback"); const selected = await prisma.application.findFirst({ where: { gigId: gig.id, status: ApplicationStatus.SELECTED }, select: { studentId: true } }); const [, updated] = await prisma.$transaction([prisma.revisionRequest.create({ data: { gigId: gig.id, feedback } }), prisma.gig.update({ where: { id: gig.id }, data: { status: GigStatus.REVISION_REQUESTED }, include: gigInclude })]); if (selected) notifySafely({ userId: selected.studentId, type: NotificationType.GIG_STATUS_CHANGED, message: `Changes requested on "${gig.title}"`, relatedGigId: gig.id }); return res.status(201).json({ gig: updated }); } catch (error) { return res.status(400).json({ message: error instanceof Error ? error.message : "Could not request revision" }); } }

export async function approveGig(req: Request, res: Response) { return res.status(410).json({ message: "Use the release-payment endpoint to approve submitted work and release its held payment" }); }
