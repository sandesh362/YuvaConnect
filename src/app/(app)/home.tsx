...
export default function HomeScreen() {
  ...
  return <SafeAreaView style={styles.safe}><View style={styles.card}><Text style={styles.brand}>YuvaConnect</Text><Text style={styles.heading}>Hello, {currentUser.name}</Text><Text style={styles.role}>{isStudent ? 'Student' : 'Business'} account</Text><Text style={styles.copy}>Your profile helps local businesses and students get to know you.</Text><View style={styles.actions}><Button title="View and edit profile" onPress={openProfile} /><Button title={isStudent ? 'Find gigs' : 'Post a gig'} onPress={() => router.push((isStudent ? '/(student)/feed' : '/(business)/post-gig') as never)} /><Button title="My gigs" variant="outline" onPress={() => router.push((isStudent ? '/(student)/my-gigs' : '/(business)/my-gigs') as never)} /><Button title="Sign out" variant="outline" onPress={async () => { await signOut(); router.replace('/login' as never); }} /></View></View></SafeAreaView>;
}
