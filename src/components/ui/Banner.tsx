...
export function Banner({
  ...
  // A metric plus copy in one row starves the copy on narrow phones, so the
  // trailing column drops onto its own row when the banner also has a metric.
  // (Declared before `content` — the JSX reads it while building the tree.)
  const hasTrailingRow = !!metric && !compact;
    ...
    <>
      ...
      <View style={styles.body}>
        ...
        {description ? (
          <Text
            variant="caption"
            style={{ color: gradientForegroundMuted, marginTop: 2 }}
          >
            {description}
          </Text>
        ) : null}
      ...
      {/* Trailing slot: a metric on its own row would waste the row, so the
          metric and the action pill share one trailing column. */}
      {metric || (actionLabel && onAction) || onDismiss ? (
        <View style={[styles.trailing, hasTrailingRow && styles.trailingRow]}>
          {metric ? (
          ...
          {actionLabel && onAction ? (
            ...
            >
              <Text
                variant="caption"
                style={{ color: gradientForeground, fontWeight: "700" }}
              >
                {actionLabel}
              </Text>
  ...
  if (tone === "flat") {
    return (
      <View
        ...
        style={[
          styles.flat,
          hasTrailingRow && styles.rowWrap,
          { padding, borderRadius: corner },
          style,
        ]}
      >
        {content}
  ...
  const containerStyle: StyleProp<ViewStyle> = [
    styles.container,
    { borderRadius: corner, marginBottom: space.md },
    style,
  ];
...
export function InfoBanner({
  ...
  const skins = {
    info: {
      ...
      fg: color.info,
      icon: "infoFilled" as IconName,
    },
    success: {
      bg: color.successSoft,
      border: color.successBorder,
      fg: color.successStrong,
      icon: "shieldCheckFilled" as IconName,
    },
    warning: {
      bg: color.warningSoft,
      border: color.warningBorder,
      fg: color.warningStrong,
      icon: "alertFilled" as IconName,
    },
    danger: {
      bg: color.dangerSoft,
      border: color.dangerBorder,
      fg: color.dangerStrong,
      icon: "alertFilled" as IconName,
    },
    neutral: {
  ...
  return (
    <View
      style={[
        styles.info,
        { backgroundColor: skins.bg, borderColor: skins.border },
        style,
      ]}
    >
      ...
      <View style={styles.body}>
        ...
        {description ? (
          <Text
            variant="caption"
            style={{ color: skins.fg, opacity: 0.85, marginTop: 1 }}
          >
            {description}
          </Text>
        ) : null}
...
const styles = StyleSheet.create({
  ...
  /* Used when the banner carries a metric: the trailing block drops to its own line. */
  ...
  body: { flex: 1, flexShrink: 1, minWidth: 0, gap: 0 },
  trailing: { alignItems: "flex-end", gap: space.sm },
  trailingRow: {
    flexBasis: "100%",
    flexGrow: 1,
    flexDirection: "row",
    // Wraps on narrow phones so the action pill moves below the metric rather
    // than being pushed off the card.
    flexWrap: "wrap",
