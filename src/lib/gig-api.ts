...
export async function updateGig(token: string, id: string, input: Partial<GigInput>) { const { data } = await api.patch<{ gig: Gig }>(`/api/gigs/${id}`, input, headers(token)); return data.gig; }
...
export async function getApplicants(token: string, gigId: string) { const { data } = await api.get<{ applicants: (Application & { student: UserSummary; avgRating: number; totalRatings: number; pastGigCount: number })[] }>(`/api/gigs/${gigId}/applicants`, headers(token)); return data.applicants; }
type UserSummary = { id: string; name: string; email: string; studentProfile?: { college: string; skills: string[]; bio: string; profileImageUrl: string | null; avgRating: number; totalRatings: number } | null };
